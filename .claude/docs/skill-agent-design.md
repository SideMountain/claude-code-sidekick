...
> 遅延ロード doc・常駐しない。スキル / エージェントを新規作成・改修するときに Read する。読者: 下流プロジェクトの Claude（runtime）+ ccs 保守者 · 役割: 仕様。
...
| 機能 | 状態 | メモ |
|---|---|---|
| `context: fork` | ✅ **公式・安定**（旧 #18394 の不安定は解消） | スキルを独立コンテキストの隔離サブエージェントとして実行。**ただし WSL 等の一部環境での成功率は採用前に都度確認**（ccs 規律） |
