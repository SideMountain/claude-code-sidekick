...
**「存在する・生きている・効いている」の主張は、履歴や伝聞の引用ではなく、現在状態に対して当該セッションで走らせた決定的オラクルで裏付ける。**

- **時点根拠 ≠ 現在の真**: 追記型履歴（DB migration・CHANGELOG・ADR 系列）では、履歴のどの 1 点を引用しても現状の証明にならない。過去の CREATE 定義は、後続の DROP / RENAME で無効化されているかもしれない。正しいオラクルは履歴の目視 replay ではなく、**導出済みの現在状態**（機械が履歴を replay した結果・実環境への照会・生成型）。
- **subagent の報告は伝聞**: 子が本物の一次ソースを引用していても、それが「現在の」状態である保証はない。load-bearing な事実（不可逆操作の土台になる事実）は、親が自分のオラクルで確定するまで確証に昇格させない。
- **発火は操作クラスで固定する**: 「この事実は load-bearing か?」の判断自体が誤りうるため、migration / DDL / スキーマ変更を書く操作では**参照する全オブジェクトの現存確認を無条件に必須**とする（判断を挟まない。H1「毎回実行」と同じ構造）。
