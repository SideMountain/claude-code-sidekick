...
リリースノートに**下流 PJ オーナーへの手動手順**を含めない。下流 PJ オーナーは `/inventory` と `/adopt-sidekick-update` の実行だけで取り込みが完結すべきである（ADR-0015 の完結性原則）。

### 禁止される記述例

- 「下流 PJ で `rm docs/decisions/XXXX.md` を実行してください」
- 「`docs/decisions/README.md` を手動で復元してください」
- 「設定ファイルを手動で編集してください」

### 代わりに

新しい変更で下流 PJ 側の操作が必要になる場合、その操作を**スキル / hook 側で機械化してからリリースする**。手動手順をリリースノートに書きたくなった時点で、それは「リリース前にスキル拡張で吸収すべき変更」のサインである。

例外: 「概念的な周知」（`v0.7 で導入された機能はこれ` 等の説明）は OK。**操作を強いない記述**であれば問題ない。
...
### 判定手順（上から順に・最初に該当した severity を採る）

1. **Critical**（1 つでも YES）:
   - ガード / hook の無効化・迂回可能性を修正するか？
   - データ喪失リスクを修正するか？
   - 保護機構の閾値・既定値を変更するか？
   - セキュリティ（認可・信頼境界・入力検証）を修正するか？
   - セッション停止・可用性喪失を伴うバグ（無限ループ・ハング・stop hook 暴走等）を修正するか？
2. **Enhancement**: 全変更が opt-in または新規追加のみか（既存挙動の変更がゼロ）？
3. **Standard**: 上記のいずれにも当たらない（既定）。

「迷ったら上位（Critical > Standard > Enhancement）」は維持するが、**迷った場合はその理由を 1 行残す**（severity 判定の学習データにする）。

以下の「該当する例」は判定手順の補強（各 severity の具体イメージ）。
