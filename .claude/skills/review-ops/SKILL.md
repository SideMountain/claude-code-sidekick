...
- **今すぐの代替**: `/review` を実行する。障害モード・並行性・外部依存の耐障害性・ランタイム PII・エラー握りつぶしの観点は **REVIEW.md §1g/§1h/§1i/§1d と fitness（`review-fitness.sh` の空 catch 検出）**でカバーする（ランタイム PII は pre-commit の対象外＝§1i でレビュー）。
