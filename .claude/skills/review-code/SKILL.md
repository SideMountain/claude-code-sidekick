...
- **今すぐの代替**: `/review` を実行する（決定的 fitness → 公式 `/code-review` → REVIEW.md の PJ 規範 → min() 総合判定）。実装整合性・DB 設計・水平展開・セキュリティの観点は **REVIEW.md §1 と `/code-review` の effort**（high / ultra）でカバーする。
