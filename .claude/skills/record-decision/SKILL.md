...
**書く前に、リポ内の直近の良質な ADR を 1 本 Read して構造をアンカーにする**（コピペ例文より実物の方が陳腐化しない）。踏襲する構造: ①ステータス（採否 + 日付 + 前提 ADR）②背景（課題を客観的技術問題として・時系列描写をしない, `.claude/rules/oss-doc-authoring.md`）③決定（番号付き・各決定に理由を内包）④選択肢と却下理由（技術的理由とセット）⑤検証（実機で確認した内容を列挙・「動くはず」を書かない）。
（アンカーは自リポの最新の良質な ADR を使う。sidekick 本体では ADR-0025 が good exemplar — sidekick の ADR は下流に配布されない設計のため、下流 PJ は自リポの ADR をアンカーにする。）
