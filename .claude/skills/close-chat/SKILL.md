...
「積む」と判断された項目を **`BACKLOG.md`**（auto-memory 内・`MEMORY.md` とは別ファイル）に追記する。
...
**追記ルール:**
- 追記は `Edit` ツールで **`BACKLOG.md`** に行う。**`MEMORY.md` に backlog を書かない**
  （`MEMORY.md` は毎セッション常駐するため、積み残しを置くと常駐コストを食う・ADR-0035）
- 既存のバックログ項目は削除しない（新規追記のみ）
- コンテキストは「次チャットでゼロから読んでも意味がわかる」程度に書く
- ⚠️ **`［Issue推奨］`と書いて済ませない。** そう判定したなら**その場で `gh issue create` する**。
  判定だけして起票しないと、backlog が Issue の代替物として溜まる

**`MEMORY.md` に書くのは Active Work だけ。1 エントリ 1 行に収める。**
詳細（実測値・判断の経緯・落とし穴）は topic ファイルに書き、Active Work からは `[[topic 名]]` で参照する。
1 エントリが数行に膨らんだら、それは topic に置くべき内容が混ざっているサイン。
...
- **新規タスク投入（即時）**: オーナーの判断が必要なものは **即座にタスクDBへ投入する**
  ...
  - オーナーの判断不要な細かいものは `BACKLOG.md` に記録

#### 同期しないもの
- `BACKLOG.md` の細かい調査メモ・検討残（Claude内部の作業コンテキスト）
- オーナーの判断が不要な軽微な作業
...
- ADR化されなかった軽微な判断も含む（再利用可能な知見として記録するため）
...
**`BACKLOG.md` は自動ロードされない**（常駐コストを避けるため意図的に分けてある・ADR-0035）。
次チャットで backlog を見るのは `/inventory` を実行したとき、またはユーザーが「バックログ確認して」と
言ったときで、そのタイミングで一覧を提示する。

バックログ項目が完了したら、`BACKLOG.md` の対応行を削除する（チェックマークではなく削除）。
...
- **バックログ肥大化** — 「念のため積んでおく」を繰り返すと `BACKLOG.md` が膨らむ。積む前に「次回のセッションで本当に必要か」を自問する。⚠️ **肥大の主因はサイズではなく振り分け漏れ**（Issue クラスが backlog に溜まる）。「他人が拾えるか」で判定し、YES ならその場で起票する
