...
知識還流候補（思想・判断・学び）:
  L0 候補（業界共通の判断軸 → ccs 還流候補）:
  1. [ ] 内容 → brain L0 のどのセクションに影響するか
  L1 候補（個人の判断軸 → 複数 PJ 横断）:
  1. [ ] 内容 → ホーム L1 への昇格候補
  L2 固有（この PJ ドメイン依存）:
  1. [ ] 内容 → PJ L2 への昇格 or feedback_*.md or ADR
...
**重要: 即座に brain ファイル (L0/L1/L2) を更新しない。** フラグを立てるだけ。統合は棚卸し時にまとめてやる。
...
`[L0候補]` `[L1候補]` `[L2固有]` のプレフィックス付き項目は、通常のバックログとは性質が異なる:
- **棚卸し時にまとめて処理する**
