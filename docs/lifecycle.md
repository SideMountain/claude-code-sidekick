...
| `.claude/scripts/distribution-gate.sh` | Fail-closed caller-side decision (stop/proceed) shared by `/review` Step 0, `/inventory` 5e and `/adopt-sidekick-update` Step 6.6. Owns the exit-code mapping so no caller re-derives it in prose, and its `*)` branch makes an unanticipated status a stop rather than a pass. Checked bootstrap: never installs an empty or unexecutable verifier |
| `.claude/scripts/verify-distribution.sh` + `distribution-manifest.tsv` | Companion-completeness gate: every asset a local SKILL.md/rule/REVIEW.md references by path must exist, plus the manifest deps a path scan cannot see. Wired into `/review` Step 0 (stop before a verdict), `/inventory` 5e (detect), `/adopt-sidekick-update` Step 6.6 (`--repair-from` the tag). Exists because distribution is a diff, not a closure — Loops 3/6
| `tests/fixtures/review-fitness/` | Characterization corpus + `replay.sh` (4 invocation modes, byte-exact goldens) + `bench.sh` (synthetic N-line diff, wall time + output checksum): freezes the fitness contract so the implementation can be re-shaped without silently changing findings
| `tests/fixtures/distribution-gate/` | 12-case fail-closed regression suite for the distribution gate: companion/manifest/verifier absence, repair success, missing-at-ref-too, unexpected verifier exit, and bootstrap failures (object absent, empty object, no temp litter) |
